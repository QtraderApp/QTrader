...
class SignalEvent(Event):
    ...
    Published when strategy identifies a trading opportunity. Signals are
    evaluated by RiskService before becoming orders. This separation allows
    risk management to validate, modify, or reject signals.

    Published by: StrategyContext (Phase 6)
    Consumed by: RiskService (Phase 4), Analytics, Reporting

    Attributes:
        signal_id: Unique signal identifier
        symbol: Ticker symbol
        direction: Buy or sell signal
        ...
        metadata: Additional signal details (indicators, prices, etc.)
    ...
    Flow:
        Strategy → SignalEvent → RiskService → OrderEvent (if approved)
    """
