...
def init_library_command(path: Path, types: tuple[str, ...], force: bool) -> None:
    ...
    Examples:
        ...
        qtrader init-library ./custom --type strategy --type indicator

        # Create structure in existing directory
        qtrader init-library ./my_lib --force
    """
            ...
            if template_path.exists():
                ...
                if target_file.endswith("__init__.py"):
                    if not target_file_path.exists() or force:
                        target_file_path.write_text('"""Custom QTrader components."""\n')
    ...
    console.print("\n[dim]Tip: See src/qtrader/scaffold/library/ for example implementations[/dim]")
...
def _generate_readme(path: Path, types: set[str]) -> str:
    ...
    return f"""# Custom QTrader Library

This directory contains custom components for QTrader.
...
- [QTrader Documentation](https://github.com/QtraderApp/QTrader) - Main documentation
- See `src/qtrader/scaffold/library/` in the QTrader package for example implementations
- See `src/qtrader/templates/` for up-to-date component templates
...
Run `qtrader init-library --help` for more options.
"""
