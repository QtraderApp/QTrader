...
Defines the interface that all data adapters must implement to work
with DataService. This protocol enables adapter-agnostic data streaming
without hard-coding vendor-specific types (AlgoseekBar, etc.).
...
- See algoseek.py for reference implementation of update_to_latest().
"""
...
class IDataAdapter(Protocol):
    ...
    def read_bars(self, start_date: str, end_date: str) -> Iterator[Any]:
        ...
        Bar type is adapter-specific (AlgoseekBar, SchwabBar, etc.).
