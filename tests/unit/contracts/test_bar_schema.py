...
def test_bar_schema_has_adjustment_factor_fields(bar_schema):
    """Verify that the schema includes adjustment factor fields."""
    properties = bar_schema["properties"]

    assert "cumulative_price_factor" in properties
    assert "cumulative_volume_factor" in properties
    assert "price_adjustment_factor" in properties
    assert "volume_adjustment_factor" in properties
    assert "adjustment_reason" in properties
...
def test_bar_decimal_fields_accept_strings(bar_schema):
    ...
    valid_payload = {
        ...
        "close": "129.04",
        "volume": "210249674",
        "cumulative_price_factor": "32.402197151180104",
        "cumulative_volume_factor": "27.99999999999997",
    }
...
def test_bar_adjustment_factors_can_be_null(bar_schema):
    """Verify that per-bar adjustment factors can be null."""
    valid_payload = {
        "symbol": "AAPL",
        "asset_class": "equity",
        "interval": "1d",
