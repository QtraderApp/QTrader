...
from qtrader.services.portfolio.models import LedgerEntryType
...
class TestStockSplits:
    ...
    def test_split_no_position(
        ...
        """Test that split on non-existent position raises ValueError."""
...
class TestDividends:
    ...
    def test_dividend_no_position(
        ...
        """Test that dividend on non-existent position raises ValueError."""
        with pytest.raises(ValueError, match="No position found"):
            service.process_dividend(symbol="AAPL", ex_date=timestamp, amount_per_share=Decimal("0.82"))
