...
def test_cache_bar_stores_bar(context):
    """_cache_bar stores bar in cache."""
    # Arrange
    ...
    assert context._bar_cache["AAPL"][0] == bar


def test_cache_bar_multiple_symbols(context):
    """_cache_bar handles multiple symbols independently."""
    # Arrange
    from qtrader.events.events import PriceBarEvent
...
def test_cache_bar_sequential_bars(context):
    """_cache_bar accumulates sequential bars."""
    # Arrange
    from qtrader.events.events import PriceBarEvent
...
def test_cache_bar_respects_max_bars_limit(event_bus):
    """_cache_bar respects max_bars limit and evicts oldest."""
    # Arrange
    from qtrader.events.events import PriceBarEvent
    ...
    # Assert - only last 3 bars retained
