...
def sample_bars() -> list[Bar]:
    """Create sample bars with predictable prices."""
    bars = []
    base_time = datetime(2024, 1, 1)
    # Simple price pattern: 100, 101, 102, ...
    for i in range(50):
        bars.append(
            Bar(
...
@pytest.fixture
def flat_price_bars() -> list[Bar]:
    """Create bars with constant price (for testing stability)."""
    bars = []
    base_time = datetime(2024, 1, 1)
    for i in range(30):
        bars.append(
...
class TestSMACalculate:
    ...
    def test_sma_calculate_with_flat_prices(self, flat_price_bars: list[Bar]) -> None:
        """SMA with flat prices should return constant value."""
        # Arrange
        sma = SMA(period=10)

        # Act
        result = sma.calculate(flat_price_bars[:15])

        # Assert
        for val in result[9:]:  # After warmup
...
class TestEMACalculateAndUpdate:
    ...
    def test_ema_with_flat_prices_equals_price(self, flat_price_bars: list[Bar]) -> None:
        """EMA with flat prices should converge to price value."""
        # Arrange
        ema = EMA(period=10)

        # Act
        for bar in flat_price_bars[:15]:
            ema.update(bar)
...
class TestSMMACalculateAndUpdate:
    ...
    def test_smma_with_flat_prices_equals_price(self, flat_price_bars: list[Bar]) -> None:
        """SMMA with flat prices should equal price value."""
        # Arrange
        smma = SMMA(period=10)
